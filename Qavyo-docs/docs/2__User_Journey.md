...
### 1.2A Automatic Printing Setup
1. Owner or Manager pairs an Android tablet/phone as the branch print hub.
2. Adds LAN/Wi-Fi printers by IP address and runs test prints.
3. Adds Bluetooth printers from paired Android devices and binds each Bluetooth printer to the hub that owns the pairing.
4. Assigns Grill, Fryer, Drinks, Packing/Expedite, and POS receipt routes.
5. Confirms whether each station has its own printer or several stations share one printer.
...
### 2.2A Printer Route Management
1. Manager reviews printer health before service.
2. Updates station routes when a kitchen station moves or a printer is replaced.
3. Uses test print to verify LAN/Wi-Fi and Bluetooth printer setup.
4. Retries or reprints failed jobs when needed.
5. Understands that orders and payments continue even if a printer is offline; failed tickets are handled from the printer job list.
...
### 3.1A Printed Ticket Behavior
1. When a waiter sends an order to the kitchen, Qavyo queues station-based print jobs where printing is enabled.
2. If Grill and Fryer have separate printers, each station receives its own ticket.
3. If several stations share one printer, the printer receives one grouped ticket with station sections.
4. When payment completes, the receipt prints to the POS terminal's configured receipt printer when auto-receipt printing is enabled.
...
### 4.2 Printed Ticket Workflow
1. New kitchen orders can appear on KDS and print on paper at the same time.
2. Printed tickets follow the same station routing as the order's station tags.
3. Packing/expedite can receive a full-order summary ticket.
4. If an order is changed after a kitchen ticket prints, only the changed items should print on an update ticket.
5. If an order is voided after paper reached the kitchen, a void ticket should print.
...
## 5A. Print Hub Operator

### 5A.1 Printer Setup
1. Opens the hub app on an Android tablet or phone at the branch.
2. Enters the short pairing code created by a manager in Qavyo.
3. Hub stores its device token securely and starts polling for jobs.
4. Adds LAN/Wi-Fi printers by IP address and test prints.
5. Adds Bluetooth printers from paired Android devices and binds each Bluetooth printer to this hub.
6. Keeps the hub running as a foreground service during business hours.

### 5A.2 Daily Monitoring
1. Watches printer status for paper out, cover open, offline, or failed tickets.
2. Checks that the hub remains online.
3. Reprints or retries failed jobs from Qavyo printer management when needed.

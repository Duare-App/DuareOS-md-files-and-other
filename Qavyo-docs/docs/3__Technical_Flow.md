...
### Automatic Printing Note

Printing adds one native client to the architecture: the Flutter print hub. It
uses HTTPS to claim jobs from `Qavyo-Server`, then talks directly to LAN/Wi-Fi
or Bluetooth printers. Browser apps never open printer sockets.
...
## 7A. Automatic Printing Flow

```
Printer setup:
       -> Manager creates print hub pairing code
       -> Flutter hub redeems code at POST /api/v1/print-hub/:branchId/pair
       -> Server returns the hub token once
       -> Hub stores token securely and uses X-Print-Agent-Token afterwards

Printer administration:
       -> Manager configures printers at /api/v1/branches/:branchId/printing
       -> Printer connectionType is NETWORK or BLUETOOTH
       -> NETWORK printer stores IP and port, normally 9100
       -> BLUETOOTH printer stores Bluetooth address and is bound to one print hub agent

Routing:
       -> STATION routes map stationTag to printer
       -> EXPEDITE route prints a full-order summary
       -> RECEIPT route may be branch default or pinned to one POS terminal
       -> Several stations may share one printer; server creates one grouped job per printer

Job creation:
       -> POS order sent to kitchen or website order accepted
       -> Server creates KITCHEN_TICKET jobs after the order commit
       -> POS payment completed
       -> Server creates CUSTOMER_RECEIPT job after the payment commit
       -> Order changed after ticket printed
       -> Server creates KITCHEN_UPDATE for changed items only
       -> Order voided after ticket printed
       -> Server creates VOID_TICKET

Hub claim loop:
       -> Hub polls GET /api/v1/print-hub/:branchId/jobs/claim?limit=5
       -> Server returns QUEUED jobs this hub can print
          - NETWORK jobs are available to any online branch hub
          - BLUETOOTH jobs are available only to their bound hub
       -> Server marks jobs CLAIMED with claimLeaseUntil
       -> Hub renders payload to ESC/POS bytes
       -> Hub sends bytes to printer
       -> Hub reports PRINTED or FAILED to POST /api/v1/print-hub/:branchId/jobs/:jobId/report

Failure handling:
       -> Expired leases return CLAIMED jobs to QUEUED
       -> Failed jobs retry with backoff until maxRetries
       -> Expired jobs are marked EXPIRED and never printed
       -> Order/payment commits never roll back because of printing
```

Browser POS and Customer Website do not open printer sockets. Raw TCP,
Bluetooth Classic SPP, and future USB device access belong in the native
Flutter hub layer.
